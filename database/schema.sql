...
CREATE TABLE IF NOT EXISTS user_organizations (
    ...
    primary_org_node_id UUID,
    secondary_org_node_ids UUID[] DEFAULT ARRAY[]::UUID[],
    role VARCHAR(50),
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
-- Dynamic organization hierarchy engine
CREATE TABLE IF NOT EXISTS organization_structure_levels (
    ...
    definition_source VARCHAR(20) NOT NULL DEFAULT 'custom' CHECK (definition_source IN ('custom', 'preset')),
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    task_rollout_type VARCHAR(50) CHECK (task_rollout_type IN ('cycle_start', 'start_date')),
    recurrence_type VARCHAR(50) CHECK (recurrence_type IS NULL OR recurrence_type IN ('daily', 'weekly', 'monthly', 'quarterly', 'annually')),
    recurrence_interval INTEGER,
    escalation_status VARCHAR(50) DEFAULT 'none',
    escalation_trigger VARCHAR(32),
    escalation_days_before INTEGER DEFAULT 0,
    auto_escalate BOOLEAN DEFAULT FALSE,
    escalation_rules JSONB,
    verification_status VARCHAR(20),
    completed_by_assignee_at TIMESTAMPTZ,
    verified_by_owner_at TIMESTAMPTZ,
    deleted_at TIMESTAMPTZ,
    deleted_by UUID REFERENCES users(id) ON DELETE SET NULL,
    category VARCHAR(100) DEFAULT 'general',
    status VARCHAR(50) DEFAULT 'pending' CHECK (status IN (
        'pending', 'todo', 'active', 'in_progress', 'pending_verification',
        'completed', 'cancelled', 'overdue', 'rejected', 'deleted'
    )),
    compliance_id UUID,
    client_name VARCHAR(255),
    client_entity_id UUID,
    org_structure_node_id UUID REFERENCES organization_structure_nodes(id) ON DELETE SET NULL,
    org_structure_level_key VARCHAR(100),
    org_structure_path JSONB,
...
CREATE TABLE IF NOT EXISTS task_assignees (
    ...
    status VARCHAR(50) CHECK (status IS NULL OR status IN (
        'todo', 'inprogress', 'duesoon', 'overdue', 'completed', 'scheduled', 'exited'
    )),
