  ...
  try {
    ...
    const result = await pool.query(
      `SELECT 
        ...
        (
          CASE WHEN c.is_group = FALSE AND c.is_task_group = FALSE THEN
            (SELECT u.profile_photo_url FROM conversation_members cm2 JOIN users u ON cm2.user_id = u.id WHERE cm2.conversation_id = c.id AND cm2.user_id != $1 LIMIT 1)
          ELSE NULL END
        ) as photo_url,
        (
        ...
        (
          SELECT json_build_object(
            ...
            'sender_name', u.name,
            'status', (SELECT COALESCE(ms.status, 'sent') FROM message_status ms WHERE ms.message_id = m.id AND ms.user_id = $1 LIMIT 1),
...
router.get('/:conversationId', authenticateToken, async (req, res) => {
  try {
    ...
    const membersResult = await pool.query(
      `SELECT u.id, u.name, u.phone, u.profile_photo_url, cm.role, cm.joined_at
